...
function groupQABlocks() {
    ...
    Object.keys(categoriesConfig).forEach(groupKey => {
        ...
        // Find the category group content by ID
        const groupContent = qaContainer.querySelector(`#${groupKey}`);

        if (groupContent) {
            // Move all child nodes of the group content to the group div
            while (groupContent.firstChild) {
                groupDiv.appendChild(groupContent.firstChild);
            }
            qaContainer.appendChild(groupDiv);
        } else {
            console.warn(`No content found for group ${groupKey}`);
        }
